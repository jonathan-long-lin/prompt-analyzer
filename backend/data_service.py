...
class PromptDataService:
    ...
    def get_overview_stats(self) -> Dict[str, Any]:
        ...
        return {
            "total_prompts": convert_to_json_serializable(len(self.df)),
            "unique_users": convert_to_json_serializable(self.df["user_id"].nunique()),
            "date_range": {
                "start": convert_to_json_serializable(min_timestamp),
                "end": convert_to_json_serializable(max_timestamp),
            },
            "total_tokens": convert_to_json_serializable(self.df["tokens_used"].sum()),
            "avg_quality": convert_to_json_serializable(
                self.df["response_quality"].mean()
            ),
            "total_cost": convert_to_json_serializable(self.df["cost_usd"].sum())
    ...
    def get_user_aggregations(self, limit: int = 10) -> Dict[str, Any]:
        ...
        # Convert to list of dictionaries using the safe conversion
        result = []
        for user_id, row in user_stats.head(limit).iterrows():
            result.append(
                {
                    "user_id": convert_to_json_serializable(user_id),
                    "user_name": convert_to_json_serializable(row["user_name"]),
                    ...
                    "avg_quality": convert_to_json_serializable(
                        row["response_quality_mean"]
                    ),
                    "avg_prompt_length": convert_to_json_serializable(
                        row["prompt_length_mean"]
                    ),
                    "first_prompt": convert_to_json_serializable(row["timestamp_min"]),
                    "last_prompt": convert_to_json_serializable(row["timestamp_max"]),
                    "total_cost": convert_to_json_serializable(
                        row.get("cost_usd_sum", 0)
                    ),
                }
            )
        ...
        return {
            "users": result,
            "total_users": convert_to_json_serializable(self.df["user_id"].nunique()),
        }
    ...
    def get_temporal_analysis(self, period: str = "daily") -> Dict[str, Any]:
        ...
        if period == "hourly":
            ...
            temporal_data = self.df.groupby("hour").agg(
                {"prompt": "count", "tokens_used": "sum", "response_quality": "mean"}
            )
                ...
                result.append(
                    {
                        ...
                        "period_value": convert_to_json_serializable(hour),
                        "prompt_count": convert_to_json_serializable(
                            row["prompt_count"]
                        ),
                        "total_tokens": convert_to_json_serializable(
                            row["tokens_used"]
                        ),
                        "avg_quality": self.safe_round(row["response_quality"], 2),
                    }
                )
                ...
                result.append(
                    {
                        ...
                        "avg_quality": self.safe_round(row["response_quality"], 2),
                        "unique_users": convert_to_json_serializable(row["user_id"]),
                    }
                )
    ...
    def get_model_performance(self) -> Dict[str, Any]:
        ...
        for model, row in model_stats.iterrows():
            result.append(
                {
                    "model": convert_to_json_serializable(model),
                    "prompt_count": convert_to_json_serializable(row["prompt_count"]),
                    "total_tokens": convert_to_json_serializable(
                        row["tokens_used_sum"]
                    ),
                    "avg_tokens": self.safe_round(row["tokens_used_mean"], 1),
                    "avg_quality": self.safe_round(row["response_quality_mean"], 2),
                    "avg_response_time": self.safe_round(
                        row.get("response_time_ms_mean", 0), 0
                    ),
                    "total_cost": self.safe_round(row.get("cost_usd_sum", 0), 3),
                    "usage_percentage": self.safe_round(
        ...
        # Sort by usage count
        result = sorted(result, key=lambda x: x["prompt_count"] or 0, reverse=True)
    ...
    def get_category_analysis(self) -> Dict[str, Any]:
        ...
        for category, row in category_stats.iterrows():
            result.append(
                {
                    "category": convert_to_json_serializable(category),
                    "prompt_count": convert_to_json_serializable(row["prompt_count"]),
                    "avg_tokens": self.safe_round(row["tokens_used"], 1),
                    "avg_quality": self.safe_round(row["response_quality"], 2),
                    "avg_prompt_length": self.safe_round(row["prompt_length"], 1),
                    "usage_percentage": self.safe_round(
                        (row["prompt_count"] / len(self.df)) * 100, 1
                    ),
                }
    ...
    def get_quality_insights(self) -> Dict[str, Any]:
        ...
        insights = {
            ...
            "avg_quality": self.safe_round(self.df["response_quality"].mean(), 2),
            "quality_std": self.safe_round(self.df["response_quality"].std(), 2),
            "low_quality_count": convert_to_json_serializable(len(low_quality)),
            "low_quality_characteristics": {},
        }

        if not low_quality.empty:
            most_common_category = low_quality["category"].mode()
            most_common_model = low_quality["model"].mode()

            insights["low_quality_characteristics"] = {
                "avg_prompt_length": self.safe_round(
                    low_quality["prompt_length"].mean(), 1
                ),
                "most_common_category": convert_to_json_serializable(
                    most_common_category.iloc[0]
                    if not most_common_category.empty
                    else "N/A"
                ),
                "most_common_model": convert_to_json_serializable(
                    most_common_model.iloc[0] if not most_common_model.empty else "N/A"
                ),
            }
